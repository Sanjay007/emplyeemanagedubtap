...
import { VerificationReport, VERIFICATION_STATUSES } from "@shared/schema";
...
export default function AdminVerificationManagement({ user }: AdminVerificationManagementProps) {
  ...
  const filterByStatus = (reports: VerificationReport[], status: string) => {
    return reports.filter(report => report.status === status);
  };

  // Function to filter reports by search query
  const filterBySearch = (reports: VerificationReport[], query: string) => {
  ...
  // Open reject dialog
  function openRejectDialog(report: VerificationReport) {
    setSelectedReport(report);
    setRejectionReason("");
  ...
  // Count by status
  const pendingCount = allReports.filter((r: VerificationReport) => r.status === VERIFICATION_STATUSES.PENDING).length;
  const approvedCount = allReports.filter((r: VerificationReport) => r.status === VERIFICATION_STATUSES.APPROVED).length;
  const rejectedCount = allReports.filter((r: VerificationReport) => r.status === VERIFICATION_STATUSES.REJECTED).length;
...
interface VerificationTableProps {
  ...
  onViewClick: (report: VerificationReport) => void;
  onApproveClick?: (report: VerificationReport) => void;
  onRejectClick?: (report: VerificationReport) => void;
  showActions: boolean;
}
  ...
  return (
    <Table>
      ...
      <TableBody>
        {reports.length === 0 ? (
          <TableRow>
            <TableCell colSpan={reports.some(r => r.status === VERIFICATION_STATUSES.REJECTED) ? 7 : 6} className="text-center py-6">
              No verification reports found.
            </TableCell>
          </TableRow>
        ) : (
