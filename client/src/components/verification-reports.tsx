...
function exportToCSV(data: any[], filename: string) {
  ...
  // Add bdeInfo.name and bdmInfo.name columns instead of bdeId and bdmId
  const headerIndex = headers.indexOf('bdeId');
  if (headerIndex !== -1) {
    headers.splice(headerIndex, 1, 'bdeName');
  }
  
  const headerBdmIndex = headers.indexOf('bdmId');
  if (headerBdmIndex !== -1) {
    headers.splice(headerBdmIndex, 1, 'bdmName');
  }
  
  // Format the headers for CSV
  ...
  data.forEach(item => {
    const row = headers.map(header => {
      if (header === 'bdeName') {
        return item.bdeInfo ? `"${item.bdeInfo.name}"` : '""';
      } else if (header === 'bdmName') {
        return item.bdmInfo ? `"${item.bdmInfo.name}"` : '""';
      } else if (header === 'date') {
...
export default function VerificationReports({ user }: VerificationReportsProps) {
  ...
  const {
    data: verificationReports = [] as ExtendedVerificationReport[],
    isLoading,
    refetch
  ...
  function editReport(report: ExtendedVerificationReport) {
    setSelectedReport(report);
      ...
      <Card className="mb-6">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex space-x-2">
            ...
            <Button 
              variant="outline" 
              onClick={() => exportToCSV(verificationReports, 'verification-reports')}
              disabled={verificationReports.length === 0}
            >
            ...
            <Table>
              ...
              <TableHeader>
                <TableRow>
                  ...
                  {(user.userType === USER_ROLES.ADMIN || user.userType === USER_ROLES.MANAGER || user.userType === USER_ROLES.BDM) && (
                    <>
                      <TableHead>BDE Name</TableHead>
                      {user.userType !== USER_ROLES.BDM && <TableHead>BDM Name</TableHead>}
                    </>
                  )}
              ...
              <TableBody>
                {verificationReports.length === 0 ? (
                  <TableRow>
                    <TableCell 
                      colSpan={
                        user.userType === USER_ROLES.ADMIN ? 9 : 
                        user.userType === USER_ROLES.MANAGER ? 8 : 
                        user.userType === USER_ROLES.BDM ? 7 : 6
                      } 
                  ...
                  verificationReports.map((report: ExtendedVerificationReport) => (
                    <TableRow key={report.id}>
                      ...
                      {(user.userType === USER_ROLES.ADMIN || user.userType === USER_ROLES.MANAGER || user.userType === USER_ROLES.BDM) && (
                        <>
                          ...
                          {user.userType !== USER_ROLES.BDM && <TableCell>{report.bdmInfo ? report.bdmInfo.name : "N/A"}</TableCell>}
                        </>
                      )}
                      <TableCell>
